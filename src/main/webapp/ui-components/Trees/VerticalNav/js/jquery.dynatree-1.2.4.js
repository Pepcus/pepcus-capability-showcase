...
DynaTreeNode.prototype = {
	...
	_getInnerHtml: function() {
		...
		if( level < opts.minExpandLevel ) {
			if(level > 1){
				//res += cache.tagConnector;  // For invesco.. we don't require dots
			}
			// .. else (i.e. for root level) skip expander/connector altogether
		} else if( this.hasChildren() !== false ) {
			if (this.getLevel()==1) // for invesco .. show + symbol only for parent
				res += cache.tagExpander;
		} else {
			//res += cache.tagConnector; // For invesco.. we don't require dot
		}
		// Checkbox mode
		if( opts.checkbox && data.hideCheckbox !== true && !data.isStatusNode ) {
			if (this.getLevel()!=1) // for invesco .. do not show check box for root
...
//		this.tree.logDebug("%s.render(%s)", this, useEffects);
		// ---
		if (this.getLevel() > 2) {return;}  // for invesco : don't render level 3, (symbols)
		var tree = this.tree,
			parent = this.parent,
			data = this.data,
			opts = tree.options,
...
//				this.span.innerHTML = this._getInnerHtml();
			...
			if (isSelectedOption) {
				this.li.className = " dynatree-li-selected";
			}
...
//        this.tree._changeNodeList("select", this, sel);
		...
		if( sel ) {
			...
			if (this.getLevel()==2)  // for invesco : select row
				$(this.li).addClass(opts.classNames.liSelected);
			...
			if (this.getLevel()==2)  // for invesco : deselect LI
				$(this.li).removeClass(opts.classNames.liSelected);
...
//		this.logDebug("reactivate %o", node);
	...
	getSelectedNodes: function(stopOnParents) {
		...
		this.tnRoot.visit(function(node){
			if( node.bSelected ) {
				if (node.getLevel() == 2) // customized for invesco. Get only symbols
					{nodeList.push(node); node1 = node.parent; nodeList.push(node1);} //for invesco - add parent node as well
...
//				self.logDebug("%o", title);
			...
			if( $li.attr("title") ){
				...
				data.title = $li.attr("title"); // overrides <a title='...'>
...
//    cookieId: "dynatree-cookie", // Choose a more unique name, to allow multiple trees.
	cookieId: "invesco-dynatree", // Choose a more unique name, to allow multiple trees.
	cookie: {
...
//		disabled: "dynatree-disabled",
		...
		liSelected: "dynatree-li-selected",  //for invesco - show node selected
		lazy: "dynatree-lazy",
