...
(function($){
    $.fn.smartCart = function(options) {
        ...
        return this.each(function() {
                ...
                var elmPLSearchDiv1 = $('<div></div>').addClass("scSearchOption");
                if(options.enableSearch){
                  var btSearch = $('<a>'+labelSearchButton+'</a>').attr("href","#").addClass("scSearch btn").attr("title","Search Product");                
                  $(btSearch).click(function() {
                     var searcStr = $(txtSearch).val();                      
                     populateProducts(searcStr);
                     return false;
                  }); 
                  var btClear = $('<a>'+labelClearButton+'</a>').attr("href","#").addClass("scSearch btn").attr("title","Clear Search");
                  ...
                  var lblSearch = $('<label>'+labelSearchText+':</label>').addClass("label");                                                 
                  elmPLSearchDiv1.append(lblSearch).append(txtSearch).append(btSearch).append(btClear); 
                  elmPLSearchPanel.append(elmPLSearchDiv1);               
                }
                // Create Category filter
                var elmPLSearchDiv2 = $('<div></div>').addClass("scSelectOption");
                if(options.enableCategoryFilter){
                  var lblCategory = $('<label>'+labelCategoryText+':</label>').addClass("scLabelCategory");
				          var tempCategory = lblCategory;
                  var elmCategory = $("<select></select>").addClass("scSelCategory");                
                  elmPLSearchDiv2.append(tempCategory).append(elmCategory);     
                  elmPLSearchPanel.append(elmPLSearchDiv2);           
                  fillCategory();
                }
                ...
                var btCheckout = $('<a>'+labelCheckout+'</a>').attr("href","#").addClass("scCheckoutButton btn");                
                $(btCheckout).click(function() {
                   if($.isFunction(options.onCheckout)) {
                      ...
                      options.onCheckout.call(this,elmProductSelected);
                	   ...
                	   //alert("Checkout is disabled for now. jquery.smartCart-2.0.js line #134");
                      $(".checkoutPopupOuterDiv").css("display","block");     
                       $(".price-payment").html($(".scLabelSubtotalValue").html());   
                       checkoutPopup();      

                       $(".close-preview").click(function() {
                        $(".checkoutPopupOuterDiv").css("display","none");
                        });     
                   }
                   return false;
                });
                elmBottomBar.append(elmBottomSubtotalText).append(elmBottomSubtotalValue).append(btCheckout);
                ...
                function addToCart(i,qty){
                   $(".scMain ul.scMenuBar li:nth-child(2) a").css({"background":"#FFFACD","transition":"all 0.2s","transform":"scale(1.05)"});
                    setTimeout(function() {
                    $(".scMain ul.scMenuBar li:nth-child(2) a").css({"background":"","transition":"all 0.2s","transform":"scale(1)"});
                    }, 500);
                ...
                function addCartItemDisplay(objProd,Quantity){
                  //  $(".scMain ul.scMenuBar li:nth-child(2) a").css("background","yellow");
                ...
                function populateProducts(searchString){
                   ...
                   $(products).each(function(i,n){
                      ...
                      if(isValid && isCategoryValid) {
                          ...
                          var btAddToCart = $('<a>'+labelAddToCartButton+'</a>').attr("href","#").attr("rel",i).attr("title",labelAddToCartButton).addClass("scAddToCart btn");
                          ...
                          var labelQty = $('<label>'+labelQuantityText+':</label>').addClass("scLabelQuantity label");                    
...
function checkoutPopup() {
 ...
 ///////////////////////////////////////////////////////////////////////////////////////
    ...
    $.fn.smartCart.defaults = {
          ...
          productItemTemplate:'<h6 class="h6"><%=pname%></h6><h4 class="label">Category: <%=pcategory%></h4><p class=""><%=pdesc%></p><h6 class="h6">Price: <%=pprice%></h6>',
